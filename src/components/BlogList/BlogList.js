import React from "react";
import Pagination from "../Pagination/Pagination";
import { Link } from "gatsby-plugin-intl";
const BlogList = ({posts, pageContext}) => {
   
    return ( 
        <div className="position-relative z-index-1 padding-y-xl">
            <div className="container max-width-adaptive-lg">
                <div className="grid gap-lg">
                <article className="story story--featured">
                    <a className="story__img radius-md" href="#0">
                    <figure className="aspect-ratio-4:3">
                        <img src="../../../app/assets/img/article-gallery-img-1.jpg" alt="Image description" />
                    </figure>
                    </a>

                    <div className="story__content">
                    <div className="margin-bottom-xs">
                        <a className="story__category" href="#0">
                        <svg className="icon margin-right-xxxs" aria-hidden="true" viewBox="0 0 16 16"><g stroke-width='1' stroke='currentColor' fill='none' stroke-linecap='round' stroke-linejoin='round'><circle cx='8' cy='7' r='1.5'></circle><path d='M12.121,14.263a7.5,7.5,0,1,0-8.242,0'></path><path d='M12.377,11.32a5.5,5.5,0,1,0-8.754,0'></path><path d='M6.605,10.5H9.4a1,1,0,0,1,1,1.1L10,15.5H6l-.39-3.9A1,1,0,0,1,6.605,10.5Z'></path></g></svg>
                        <i>Podcast</i>
                        </a>
                    </div>
            
                    <div className="text-component">
                        <h2 className="story__title"><a href="#0">{posts[0].title}</a></h2>
                        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. At iusto sit quasi aliquam quod laboriosam. Nobis, modi repellendus.</p>
                    </div>
            
                    <div className="story__author margin-top-sm">
                        <a className="block" href="#0">
                        <img src="../../../app/assets/img/article-gallery-img-author-1.svg" alt="Author picture" />
                        </a>

                        <div className="line-height-xs">
                        <address className="story__author-name"><a href="#0" rel="author">Olivia Gribben</a></address>
                        <p className="story__meta"><time>May 16</time> &mdash; 5 min read</p>
                        </div>
                    </div>
                    </div>
                </article>
                {posts.slice(1).map((element) => (
                    <article className="story col-4@md">
                        <a className="story__img radius-md" href="#0">
                        <figure className="aspect-ratio-4:3">
                            <img src="../../../app/assets/img/article-gallery-img-2.jpg" alt="Image description" />
                        </figure>
                        </a>

                        <div className="story__content">
                        <div className="margin-bottom-xs">
                            <a className="story__category" href="#0">
                            <svg className="icon margin-right-xxxs" aria-hidden="true" viewBox="0 0 16 16"><g stroke-width='1' fill='none' stroke='currentColor' stroke-linecap='round' stroke-linejoin='round'><rect x='0.5' y='1.5' width='15' height='13' rx='1' ry='1'></rect><polygon points='5.5 4.5 11 8 5.5 11.5 5.5 4.5'></polygon></g></svg>
                            <i>Video</i>
                            </a>
                        </div>
                
                        <div className="text-component">
                            <h2 className="story__title"><a href="#0">{element.title}</a></h2>
                            
                        </div>
                
                        <div className="story__author margin-top-sm">
                            <a className="block" href="#0">
                            <img src="../../../app/assets/img/article-gallery-img-author-2.svg" alt="Author picture" />
                            </a>

                            <div className="line-height-xs">
                            <address className="story__author-name"><a href="#0" rel="author">James Powell</a></address>
                            <p className="story__meta"><time>May 12</time> &mdash; 5 min read</p>
                            </div>
                        </div>
                        </div>
                    </article>
                 ))}

               

               
                </div>

                <Pagination pageContext={pageContext} />
            </div>
            </div>

        
               
           
     );
}
 
export default BlogList;