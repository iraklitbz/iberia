  ...
  <div class="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-20">
    <div class="w-full max-w-md">
      ...
      <div class="mb-8 text-center">
        ...
        <h1 class="font-display text-2xl font-bold text-zinc-900">{{ $t('auth.resetPasswordTitle') }}</h1>
        <p class="mt-2 text-sm text-zinc-500">{{ $t('auth.resetPasswordSubtitle') }}</p>
      </div>

      <!-- No code error -->
      <div v-if="!code" class="rounded-xl border border-red-100 bg-red-50 px-5 py-6 text-center">
        <p class="text-sm text-red-600">{{ $t('auth.resetInvalidLink') }}</p>
        <NuxtLink :to="localePath('/forgot-password')" class="mt-3 inline-block text-sm font-medium text-iberia hover:underline">
      ...
      <div v-else-if="success" class="rounded-xl border border-green-100 bg-green-50 px-5 py-6 text-center">
        ...
        <h2 class="font-semibold text-zinc-900">{{ $t('auth.resetSuccessTitle') }}</h2>
        <p class="mt-1 text-sm text-zinc-500">{{ $t('auth.resetSuccessMessage') }}</p>
        <NuxtLink
      ...
      <form v-else class="space-y-4" @submit.prevent="handleReset">
        ...
        <div>
          <label class="mb-1.5 block text-sm font-medium text-zinc-700">{{ $t('auth.newPassword') }}</label>
          <input
...
async function handleReset() {
  ...
  error.value = ''
  try {
    await resetPassword(code.value, form.password, form.confirmPassword)
    success.value = true
  }
  catch (err: unknown) {
